...
def vec_to_axis_vec(vec_in, vec_out):
    '''Make **vec_out** to be an axis-aligned unit mathutils.Vector that is closest to vec_in. (basis?)'''
    x, y, z = vec_in
    if abs(x) > abs(y):
        if abs(x) > abs(z):
...
def calc_bone_rotation(psk_bone, bone_len, bDirectly, avg_bone_len):
    ...
    vecy = mathutils.Vector((0.0, 1.0, 0.0))
    ...
    if len(children) == 0:
        ...
        if psk_bone.parent == None:
            ...
            if psk_bone.orig_loc.length < 0.1 * avg_bone_len:
                ...
                vecy = mathutils.Vector((1.0, 0.0, 0.0))
            else:
                # @
                # vec_to_axis_vec(psk_bone.orig_quat.conjugated() * psk_bone.parent.axis_vec, axis_vec)
                v = psk_bone.parent.axis_vec.copy()
...
def pskimport(filepath,
    ...
    if bImportbone:  #else?
        ...
        for counter, (name_raw, flags, NumChildren, ParentIndex, #0 1 2 3
            ...
            psk_bone.orig_quat = mathutils.Quaternion((quat_w, quat_x, quat_y, quat_z))

            if bScaleDown:
                psk_bone.orig_loc = mathutils.Vector((vec_x * 0.01, vec_y * 0.01, vec_z * 0.01))
            else:
                psk_bone.orig_loc = mathutils.Vector((vec_x, vec_y, vec_z))
            ...
            if psk_bone.parent_index == 0 and psk_bone.bone_index == psk_bone.parent_index:
                if bDontInvertRoot:
                    psk_bone.mat_world_rot = psk_bone.orig_quat.to_mathutils.Matrix()
                else:
                    psk_bone.mat_world_rot = psk_bone.orig_quat.conjugated().to_mathutils.Matrix()
                psk_bone.mat_world = mathutils.Matrix.Translation(psk_bone.orig_loc)

            sum_bone_pos += psk_bone.orig_loc.length
            
    
    #==================================================================================================
    # Bones. Calc World-space mathutils.Matrix
        ...
        for psk_bone in psk_bones:
            ...
            # mat_world -     world space bone mathutils.Matrix WITHOUT own rotation
            ...
            # psk_bone.mat_world_rot = parent.mat_world_rot * psk_bone.orig_quat.conjugated().to_mathutils.Matrix()
    ...
    # Skeleton. Build.
        ...
        for psk_bone in psk_bones:
            ...
            # only length of this mathutils.Vector is matter?
            edit_bone.tail = mathutils.Vector(( 0.0, new_bone_size, 0.0))

            # @
            # edit_bone.mathutils.Matrix = psk_bone.mat_world * post_quat.to_mathutils.Matrix().to_4x4()

            m = post_quat.copy()
            m.rotate( psk_bone.mat_world )

            m = m.to_mathutils.Matrix().to_4x4()
            m.translation = psk_bone.mat_world.translation

            edit_bone.mathutils.Matrix = m
            
            
            # some dev code...
            #### FINAL
            # post_quat = psk_bone.orig_quat.conjugated() * quat_diff
            # edit_bone.mathutils.Matrix = psk_bone.mat_world * test_quat.to_mathutils.Matrix().to_4x4()
            # edit_bone["post_quat"] = test_quat
            #### 

            # edit_bone["post_quat"] = mathutils.Quaternion((1,0,0,0))
            # edit_bone.mathutils.Matrix = psk_bone.mat_world* psk_bone.rot
     

            # if edit_bone.parent:
              # edit_bone.mathutils.Matrix = edit_bone.parent.mathutils.Matrix * psk_bone.trans * (psk_bone.orig_quat.conjugated().to_mathutils.Matrix().to_4x4())
              # edit_bone.mathutils.Matrix = edit_bone.parent.mathutils.Matrix * psk_bone.trans * (test_quat.to_mathutils.Matrix().to_4x4())
            # else:
              # edit_bone.mathutils.Matrix = psk_bone.orig_quat.to_mathutils.Matrix().to_4x4()
            ...
            bone = edit_bone
            if psk_bone.parent is not None:
                orig_loc  =  bone.mathutils.Matrix.translation - bone.parent.mathutils.Matrix.translation
                orig_loc.rotate( bone.parent.mathutils.Matrix.to_mathutils.Quaternion().conjugated() )

                
                orig_quat = bone.mathutils.Matrix.to_mathutils.Quaternion()
                orig_quat.rotate( bone.parent.mathutils.Matrix.to_mathutils.Quaternion().conjugated()  )
                orig_quat.conjugate()

                if orig_quat.dot( psk_bone.orig_quat ) < 0.95:
                    print(bone.name, psk_bone.orig_quat, orig_quat, orig_quat.dot( psk_bone.orig_quat ))
                    print('parent:', bone.parent.mathutils.Matrix.to_mathutils.Quaternion(), bone.parent.mathutils.Matrix.to_mathutils.Quaternion().rotation_difference(bone.mathutils.Matrix.to_mathutils.Quaternion()) )
...
def psaimport(filepath,
    ...
    def new_psa_bone(bone, pose_bone):
        ...
        if bone.get('orig_quat') is None:
            ...
            if bone.parent != None:
                ...
                psa_bone.orig_loc  =  bone.mathutils.Matrix_local.translation - bone.parent.mathutils.Matrix_local.translation
                psa_bone.orig_loc.rotate( bone.parent.mathutils.Matrix_local.to_mathutils.Quaternion().conjugated() )

                psa_bone.orig_quat = bone.mathutils.Matrix_local.to_mathutils.Quaternion()
                psa_bone.orig_quat.rotate( bone.parent.mathutils.Matrix_local.to_mathutils.Quaternion().conjugated()  )
                psa_bone.orig_quat.conjugate()
            else:
                psa_bone.orig_loc  = bone.mathutils.Matrix_local.translation.copy()
                psa_bone.orig_quat = bone.mathutils.Matrix_local.to_mathutils.Quaternion()

            psa_bone.post_quat = psa_bone.orig_quat.conjugated()
        else:
            psa_bone.orig_quat = mathutils.Quaternion(bone['orig_quat'])
            psa_bone.orig_loc  =     mathutils.Vector(bone['orig_loc'])
            psa_bone.post_quat = mathutils.Quaternion(bone['post_quat'])
    ...
    for counter, (Name, Group, Totalbones, NumRawFrames) in enumerate(Action_List):
        ...
        for psa_bone in PsaBonesToProcess:
            ...
            data_path = pose_bone.path_from_id("rotation_mathutils.Quaternion")
        ...
        for i in range(0,min(maxframes, NumRawFrames)):
            ...
            for j in range(Totalbones):
                ...
                # Set it?
                # pose_bone.rotation_mathutils.Quaternion = quat
                # pose_bone.location = loc

                    # pose_bone.rotation_mathutils.Quaternion = orig_rot.conjugated()
                    # pose_bone.location = p_pos - (pose_bone.bone.mathutils.Matrix_local.translation - pose_bone.bone.parent.mathutils.Matrix_local.translation)
